...
function listenFormSubmitEvent() {
    formElements.form.addEventListener('submit', (event) => {
        ...
        if (validateForm(fieldConfigurations)) {
            ...
            alertify.success(`Pedido del medicamento ${idTeacher ? 'modificado': 'guardado' } correctamente`);
            listTeachers();
...
function editTeacher(idTeacher) {
    ...
    if (teacher) {
        ...
        alertify.error('El pedido que seleccionaste no existe, verifique la información.');
    }
}
...
function confirmDelete(idTeacher) {
    ...
    if (teacher) {
        Swal.fire({
            title: `¿Estas seguro de que quieres eliminar el pedido del medicamento: ${teacher.name}?`,
